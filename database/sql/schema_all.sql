...
-- 創建 task_execution_logs 表格 (任務執行日誌)
...
-- 創建 task_execution_logs 索引
CREATE INDEX IF NOT EXISTS idx_task_execution_logs_task_name ON task_execution_logs(task_name);
CREATE INDEX IF NOT EXISTS idx_task_execution_logs_task_type ON task_execution_logs(task_type);
...
-- 創建 task_execution_logs 觸發器
DROP TRIGGER IF EXISTS update_task_execution_logs_updated_at ON task_execution_logs;
CREATE TRIGGER update_task_execution_logs_updated_at
...
-- 授予 anon 和 authenticated 角色對所有表格的權限
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE articles TO anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE article_chunks TO anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE embeddings_cache TO anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE sitemaps TO anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE sitemap_hierarchy TO anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE discovered_urls TO anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE robots_txt TO anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE search_logs TO anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE task_execution_logs TO anon, authenticated;

-- 授予對所有序列的權限 (用於自動生成的主鍵)
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO anon, authenticated;

-- 授予對所有函數的執行權限
GRANT EXECUTE ON FUNCTION update_updated_at_column() TO anon, authenticated;
GRANT EXECUTE ON FUNCTION update_content_hash() TO anon, authenticated;
...
GRANT EXECUTE ON FUNCTION match_chunks(VECTOR(1024), FLOAT, INT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION semantic_search(TEXT, VECTOR(1024), FLOAT, INT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION get_sitemap_hierarchy(UUID) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION get_crawl_queue(INT, TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION update_crawl_status(UUID, TEXT, UUID, TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION get_database_stats() TO anon, authenticated;
GRANT EXECUTE ON FUNCTION get_sitemap_stats() TO anon, authenticated;

DO $$
BEGIN
    RAISE NOTICE '======================================';
    RAISE NOTICE 'Supabase 權限設定完成';
    RAISE NOTICE '======================================';
END;
$$;
